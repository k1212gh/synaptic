...
pnpm add @anthropic-ai/sdk voyageai @notionhq/client
...
import { createServiceClient, createServerClient } from "@/lib/db/server";
...
export async function GET(req: NextRequest) {
  ...
  // 로그인한 Supabase 사용자 확인 — 쿠키 기반 클라이언트로 세션 읽기
  const supabase = createServerClient(); // ← 쿠키 기반, service client 아님
...
export async function listAllPages(client: Client): Promise<PageObjectResponse[]> {
...
function extractText(block: BlockObjectResponse): string {
  const b = block as BlockObjectResponse & { [key: string]: { rich_text?: { plain_text: string }[] } };
  const rich = b[block.type]?.rich_text;
...
returns table(id uuid, embedding vector) language sql stable as $$
...
export async function POST(req: Request) {
  ...
  const msg = await anthropic.messages.create({
    model: "claude-haiku-4-5-20251001", // 올바른 모델 ID
    max_tokens: 1024,
    system: "당신은 사용자의 노션 워크스페이스 전용 리서치 어시스턴트입니다. 제공된 컨텍스트 밖의 정보는 추측하지 마세요. 답변에 [인용 번호]를 반드시 붙이세요.",
    messages: [{
      role: "user",
      content: `컨텍스트:\n${(contexts ?? []).map((c: { chunk_text: string }, i: number) => `[${i+1}] ${c.chunk_text}`).join("\n---\n")}\n\n질문: ${question}`,
    }],
  });

  await supabase.from("query_logs").insert({
    user_id: user.id, query: question,
    result_chunk_ids: (contexts ?? []).map((c: { id: string }) => c.id),
